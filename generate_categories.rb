...
Motorcycle Parts & Supplies (motorcyclepartsandsupplies)
...
Crisis Pregnancy Centers (crisispregnancycenters)
Faith-based Crisis Pregnancy Centers (faithbasedcpc)
...
Reiki (reiki)
Reproductive Health Services (reproductivehealthservices)
...
Roofing (roofing)
Sauna Installation & Repair (saunainstallation)
...
Elevator Services (elevatorservices)
Engraving (engraving)
...
Notaries (notaries)
Outdoor Power Equipment Services (outdoorpowerequipmentservices)
Pest Control (pest_control)
Portable Toilet Services (portabletoiletservices)
...
Art Consultants (artconsultants)
...
Rugs (rugs)
Sheds & Outdoor Storage (shedsandoutdoorstorage)
Tableware (tableware)
...
categories_by_line = source.split("\n")
categories = []
categories_by_line.each do |l|
  category = []
  name = l.match(/.*(?= \()/)[0]
  ref = l.scan(/(?<=\().*?(?=\))/)[-1]
  categories << {name: name, ref: ref}
end

puts categories.first
puts categories.last

file = File.open("categories.yaml", "w")
